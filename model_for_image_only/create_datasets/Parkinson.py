...
def PD_Uptask_Dataset(mode, data_folder_dir="/workspace"):
    if mode == 'train':
        ...
        data_dicts   = [{"image": image_name, "seg_label": seg_label_name, "cls_label": cls_label_name} for image_name, seg_label_name, cls_label_name in zip(img_list, seg_label_list, cls_label_list)]        
        ...
        ## dummy variable if not using WeightedRandomSampler() in train.py file
        weight_vector = []
...
## Dataset for Testing #############################################################################################
def PD_TEST_Dataset(test_dataset_name, data_folder_dir="/workspace"):
