...
def train_Up_SMART_Net(model, criterion, data_loader, optimizer, device, epoch, print_freq, batch_size, freeze_feature_extractor):
    ...
    for batch_data in metric_logger.log_every(data_loader, print_freq, header):
        ...
        additional_vector = batch_data["clinical_var"].to(device)

        cls_pred, seg_pred, rec_pred = model(inputs, additional_vector)
...
## DUAL
## (CLS+SEG) & (SEG+REC) was not implemented in this version
################ CLS+REC #######################
## Train
def train_Up_SMART_Net_Dual_CLS_REC(model, criterion, data_loader, optimizer, device, epoch, print_freq, batch_size, freeze_feature_extractor):
    ...
    for batch_data in metric_logger.log_every(data_loader, print_freq, header):
        ...
        cls_pred, rec_pred = model(inputs, additional_vector)
...
def test_Up_SMART_Net_Dual_CLS_REC(model, criterion, data_loader, device, print_freq, batch_size, output_dir, idx=0):
    ...
    for batch_data in metric_logger.log_every(data_loader, print_freq, header):
        ...
        ## 김선우 선생님 방법으로 one hot encoding 하기
        cls_pred_prob_one_hot = torch.argmax(cls_pred_prob, dim=1)
        cls_pred_prob_one_hot = torch.nn.functional.one_hot(cls_pred_prob_one_hot, num_classes=2)
...
## Train
def train_Up_SMART_Net_Single_CLS(model, criterion, data_loader, optimizer, device, epoch, print_freq, batch_size, freeze_feature_extractor):
    ...
    for batch_data in metric_logger.log_every(data_loader, print_freq, header):
        ...
        cls_pred = model(inputs, additional_vector)
